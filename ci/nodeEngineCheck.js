...
class NodeEngineCheck {
   ...
   */
  constructor(config) {
    const { nodeModulesPath, packageJsonPath } = config

    // Ensure required params are set
    // eslint-disable-next-line unicorn/no-useless-undefined
    if ([nodeModulesPath, packageJsonPath].includes(undefined)) {
  ...
  async getPackageFiles(basePath = this.nodeModulesPath) {
    try {
      ...
      const validFiles = dirents
        .filter(d => d.name.toLowerCase() === 'package.json')
      ...
      throw new Error(
        `Failed to get package.json files in ${this.nodeModulesPath} with error: ${error}`
      )
    }
  }
  ...
  async getNodeVersion({ files, clean = false }) {
    ...
    for (const file of files) {
      // Get node version
      const contentString = await fs.readFile(file, 'utf8')
      try {
  ...
  getHigherVersions({ versions, baseVersion }) {
    ...
    // Sort by min version
    const sortedMinVersions = minVersions.sort((v1, v2) =>
      semver.compare(v1.nodeMinVersion, v2.nodeMinVersion)
    )
...
async function check() {
  ...
  if (parentVersion === undefined) {
    core.setFailed(
      `Failed to determine node engine version of parent package at ${this.packageJsonPath}`
    )
    return
  }
  ...
  if (higherVersions.length > 0) {
    ...
    for (const higherVersion of higherVersions) {
      ...
      console.log(
        `- ${_package} requires at least node ${higherVersion.nodeMinVersion} (${higherVersion.nodeVersion})`
      )
    }
    console.log('')
    core.setFailed(
